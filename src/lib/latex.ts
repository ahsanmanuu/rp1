...
export function autoHealLatex(latex: string): string {
  ...
  if (hasDocumentEnv) {
    ...
    if (!patchedPreamble.includes("graphicspath")) usepackageGuards.push("\\graphicspath{{./}{./figures/}{./assets/}{./images/}{../}{../figures/}{../assets/}{../images/}}");
    ...
    if (usepackageGuards.length > 0 && !patchedPreamble.includes("\\DeclareUnicodeCharacter{207B}")) {
      ...
      const postCatcodeBlock = [
        ...
        `                      \\csname includegraphics\\endcsname[#2,max height=0.7\\textheight]{../#1}%`,
        `                    }{%`,
        `                      \\IfFileExists{fallback_figure.png}{%`,
        `                        \\csname includegraphics\\endcsname[#2,max height=0.7\\textheight]{fallback_figure.png}%`,
        `                      }{%`,
        `                        \\fbox{\\parbox[c][3cm][c]{0.8\\linewidth}{\\centering [Figure: \\detokenize{#1}]}}%`,
        `                      }%`,
        `                    }%`,
        `                }%`,
        `              }%`,
