...
2. figures/charts: Generate float files for ALL manuscript figures and charts present in input B and C in document order. If a figure has a verified caption, use its verbatim caption. If a figure lacks an explicit caption in input B, provide a concise descriptive caption (e.g. "Figure N" or based on surrounding context) so the figure is never omitted. Use: \\begin{figure}[${templateConventions.floatPlacement === 'table*/figure* for wide content, [!ht] otherwise' ? '!ht' : templateConventions.floatPlacement}]\\centering\\includegraphics[width=0.9\\linewidth,max height=0.7\\textheight,keepaspectratio]{<EXACT image filename from input C, in order>}\\caption{<caption text>}\\label{fig:N}\\end{figure}
   ...
   Never omit manuscript figures or charts.
